...
def login():
	...
	try:
		...
		wel2 = mark(wel, style='red')
		sol().print(wel2)
		us=input(f"{N}[•] Masukan Username Kamu[Finn]: {N}")
...
[white][[blue]03[white]]. Crack Dari Mengikuti           [white][[red]E[white]]. [red]LogOut """,title=f"[{P2} Pilihan Menu {hapus}]",width=80,padding=(0,4),style=f"{color_table}"))
	...
	def ingfo(self):
		prints(Panel(f"\t  [white]Jangan Lupa Mainin modpes jika terendeksi[red]SPAM[/]",width=80,padding=(0,3),style=f"{color_table}"))
 ...
 {SE}╰─────────────────────────────────╯{SE} ╰───────────────────────────────────╯''',title=f"[[white] Cek Hasil [/]]",width=80,style=f"{color_table}"))
	def ifo(self):
		...
		#urut.append(Panel(f' [white][[blue]04[white]]. Method Finn V1 [white] [[green]recommend[white]]\n [white][[blue]05[white]]. Method Finn V2 [white] [[green]recommend[white]]\n [white][[blue]06[white]]. Method Finn V3 [white] [[green]recommend[white]]',width=40,title=f"[white]Method Finn",style=f"{color_table}"))
		self.tol.print(Columns(urut))
	...
	def V1(self,user,pas):
		...
		prog.update(des,description=f"[white]({Finnxyzz}✔[white]) STABIL [{Finnxyz}{user}[/]] {str(loop)}/{len(internal)} [green]OK : -{len(success)}[/] [yellow]CP : -{len(checkpoint)}[/]")
		prog.advance(des)
		for pw in pas:
				try:
...
if __name__=='__main__':
	...
	try:
		with requests.Session() as ses:
	         ko = ses.get('https://pastebin.com/raw/wXqi7abH').json()
	         HARIS.update(ko)
